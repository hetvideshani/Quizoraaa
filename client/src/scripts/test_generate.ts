...
import { generateAgentQuiz, Input } from '@/lib/aiUtils/generateQuiz';


async function main() {
  const filePath = path.join(process.cwd(), 'src', 'data', 'pdf', 'JavaScript_Interview_questions.pdf');
  // const pdfBuffer = fs.readFileSync(filePath); // Simulate uploaded PDF
  
  const input: Input = {
    pdfFile: filePath,
    prompt: "Generate a quiz for JavaScript basics for 2nd year students",
